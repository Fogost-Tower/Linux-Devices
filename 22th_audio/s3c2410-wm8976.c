...
static void wm8976_write_reg(unsigned char reg, unsigned int data)
{
	...
	// write_gpio_bit(GPIO_CSB, 1);
	s3c2410_gpio_setpin(S3C2410_GPB2,1);
	// write_gpio_bit(GPIO_SDIN, 1);
	s3c2410_gpio_setpin(S3C2410_GPB3,1);
	// write_gpio_bit(GPIO_SCLK, 1);
				...
				unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case SOUND_MIXER_WRITE_VOLUME:
			...
			/*ioctl : val 越大表示音量越大  0-最小，99最大
			*UDA1341 : 寄存器的值越小音量越大
			*/

			/*WM8976: 52.53号寄存器bit[5:0] 表示音量，值越大音量越大
			*
			*/
