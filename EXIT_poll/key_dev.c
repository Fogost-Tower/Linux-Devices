...
static int S3C2440_OPEN(struct inode *inode,struct file *file)
{
	...
	request_irq(IRQ_EINT0,S3C2440_Buttons_Irq,IRQT_BOTHEDGE,"S3",&Pin_desc[1]);
	request_irq(IRQ_EINT0,S3C2440_Buttons_Irq,IRQT_BOTHEDGE,"S4",&Pin_desc[2]);
	request_irq(IRQ_EINT0,S3C2440_Buttons_Irq,IRQT_BOTHEDGE,"S5",&Pin_desc[3]);
}
...
int S3C2440_RELEASE(struct inode *inode,struct file *file)
{
	...
	free_irq(IRQ_EINT2,&Pin_desc[0]);
	free_irq(IRQ_EINT11,&Pin_desc[0]);
	free_irq(IRQ_EINT19,&Pin_desc[0]); 
}
...
static int S3C2440_INIT(void)
{
	...
	key_class_dev= class_device_create(key_class, NULL, MKDEV(DEV_MAJOR,0), NULL,"Keydev");
